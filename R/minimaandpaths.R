...
#' minima
oneminimum<-function(inputfes, cv1=cv1, cv2=cv2) {
  fes<-inputfes$fes
  rows<-inputfes$rows
  per<-inputfes$per
  if(inputfes$dimension==2) {
...
#' @param col a list of colors such as that generated by 'rainbow',
...
#' @param nlevels number of contour levels desired *iff* 'levels' is not
#'        supplied.
