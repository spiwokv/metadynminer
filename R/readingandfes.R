...
fes.hillsfile<-function(hills, imin=1, imax=NULL, xlim=NULL, ylim=NULL, npoints=256) {
  ...
  if(hills$size[2]==7) {
    ...
    if((hills$per[1]==F)&(hills$per[2]==F)) {
      fesm<-hills1((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                   (npoints-1)*(hills$hillsfile[,3]-ylims[1])/(ylims[2]-ylims[1]-biny),
                   (npoints-1)*max(hills$hillsfile[,4])/(xlims[2]-xlims[1]-binx),
                   (npoints-1)*max(hills$hillsfile[,5])/(ylims[2]-ylims[1]-biny),
                   hills$hillsfile[,6],npoints,imin-1,imax-1)
    }
    if((hills$per[1]==T)&(hills$per[2]==F)) {
      fesm<-hills1p1((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                     (npoints-1)*(hills$hillsfile[,3]-ylims[1])/(ylims[2]-ylims[1]-biny),
                     (npoints-1)*max(hills$hillsfile[,4])/(xlims[2]-xlims[1]-binx),
                     (npoints-1)*max(hills$hillsfile[,5])/(ylims[2]-ylims[1]-biny),
                     hills$hillsfile[,6],npoints,imin-1,imax-1)
    ...
    if((hills$per[1]==F)&(hills$per[2]==T)) {
      fesm<-hills1p2((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                     (npoints-1)*(hills$hillsfile[,3]-ylims[1])/(ylims[2]-ylims[1]-biny),
                     (npoints-1)*max(hills$hillsfile[,4])/(xlims[2]-xlims[1]-binx),
                     (npoints-1)*max(hills$hillsfile[,5])/(ylims[2]-ylims[1]-biny),
                     hills$hillsfile[,6],npoints,imin-1,imax-1)
    ...
    if((hills$per[1]==T)&(hills$per[2]==T)) {
      fesm<-hills1p12((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                      (npoints-1)*(hills$hillsfile[,3]-ylims[1])/(ylims[2]-ylims[1]-biny),
                      (npoints-1)*max(hills$hillsfile[,4])/(xlims[2]-xlims[1]-binx),
                      (npoints-1)*max(hills$hillsfile[,5])/(ylims[2]-ylims[1]-biny),
                      hills$hillsfile[,6],npoints,imin-1,imax-1)
  ...
  if(hills$size[2]==5) {
    ...
    binx<-(xlims[2]-xlims[1])/(npoints-1)
    if(hills$per[1]==F) {
      fesm<-hills1d1((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                     (npoints-1)*max(hills$hillsfile[,3])/(xlims[2]-xlims[1]-binx),
                     hills$hillsfile[,4],npoints,imin-1,imax-1)
    }
    if(hills$per[1]==T) {
      fesm<-hills1d1p((npoints-1)*(hills$hillsfile[,2]-xlims[1])/(xlims[2]-xlims[1]-binx),
                      (npoints-1)*max(hills$hillsfile[,3])/(xlims[2]-xlims[1]-binx),
                      hills$hillsfile[,4],npoints,imin-1,imax-1)
    }
