...
test_that("Testing that read.hills correctly loads hills and ralated functions analyze them", {
  ...
  expect_equal(object=summyfes, expected=-4066, tolerance=1, scale=1)
  ...
  myfes<-fes(h2)
  summyfes<-sum(myfes$fes)
  expect_equal(object=summyfes, expected=-8131, tolerance=1, scale=1)
  ...
  myfes<-fes(h)
  summyfes<-sum(myfes$fes)
  expect_equal(object=summyfes, expected=-135.2, tolerance=0.1, scale=1)
